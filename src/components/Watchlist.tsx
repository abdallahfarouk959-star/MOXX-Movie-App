...
export default function Watchlist() {
  ...
  const removeItem = async (tmdbId: number) => {
    ...
    const path = `users/${auth.currentUser.uid}/watchlist/${tmdbId}`;
    try {
  ...
  if (loading) return <div className="pt-32 px-8 text-center text-gray-400">Loading your list...</div>;
  if (!auth.currentUser) return <div className="pt-32 px-8 text-center text-gray-400">Please sign in to view your watchlist.</div>;
    ...
    <div className="pt-24 sm:pt-32 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-screen">
      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-6">
          {items.map(item => (
            <div key={item.tmdbId} className="group relative aspect-[2/3] rounded-lg overflow-hidden bg-netflix-dark">
              <img 
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-center items-center gap-4">
                ...
                <p className="text-xs font-bold text-center px-2 line-clamp-2">{item.title}</p>
              </div>
            </div>
          ))}
        </div>
